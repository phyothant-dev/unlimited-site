...
export default function App() {
  ...
  const handleVote = async (project) => {
    setError(null)
    try {
      await submitVote(project.id, activeCategory)
  ...
  return (
    <div style={s.page}>
          ...
          <AnimatePresence mode="wait">
            <motion.div key={activeCategory} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={s.grid}>
              {projects.filter(p => p.title.toLowerCase().includes(search.toLowerCase())).length === 0 ? (
                ...
                <ProjectCard key={project.id} project={project} onVote={() => setConfirmProject(project)} onDetail={setDetailProject} index={i} />
              ))}
            </motion.div>
          </AnimatePresence>
        )}
      ...
      <ProjectDetail project={detailProject} onVote={(p) => { setDetailProject(null); setConfirmProject(p) }} onClose={() => setDetailProject(null)} />
